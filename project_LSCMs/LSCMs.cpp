...
using namespace inf574_project;

void LeastSquaresConformalMaps::build_xiyi_method1(const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, const Eigen::Vector3d& p3, double& x1, double& y1, double& x2, double& y2, double& x3, double& y3, double& area)
{
	...
	Eigen::Vector3d e12 = p2 - p1; // from p1 to p2
	Eigen::Vector3d e13 = p3 - p1; // from p1 to p3
	double e12_len = e12.norm();
	double e13_len = e13.norm();
	//double e23_len = (p3 - p2).norm();
	Eigen::Vector3d unit_e12 = e12.normalized();
	Eigen::Vector3d unit_e13 = e13.normalized();
	...
	area = sin_angle * e12_len * e13_len; // it is actually " e12.cross(e13) "
}

void LeastSquaresConformalMaps::build_xiyi_method2(const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, const Eigen::Vector3d& p3, double& x1, double& y1, double& x2, double& y2, double& x3, double& y3, double& area)
{
	// pick barycenter of triangle as origin, op1 as one basis
	Eigen::Vector3d center = (p1 + p2 + p3) / 3.0;
	Eigen::Vector3d cp1 = p1 - center; // center to p2
	Eigen::Vector3d cp2 = p2 - center; // center to p2
	Eigen::Vector3d cp3 = p3 - center; // center to p3
	Eigen::Vector3d normal = (cp1.cross(cp2)).normalized();

	Eigen::Vector3d basis1 = cp1.normalized();
	// rotate basis1 90 degree to get basis2
	Matrix3d rot = Matrix3d::Identity() * Eigen::AngleAxisd(M_PI_2, normal);
	Eigen::Vector3d basis2 = (rot * basis1).normalized();
	// all xi,yi are the projected part onto basis
	...
	Eigen::Vector3d e12 = p2 - p1; // from p1 to p2
	Eigen::Vector3d e13 = p3 - p1; // from p1 to p3
	area = (e12.cross(e13)).norm();
}

void LeastSquaresConformalMaps::swap_points(Eigen::MatrixXd& points, int index1, int index2)
{
...
void LeastSquaresConformalMaps::compute_pinned_uv()
{
...
void LeastSquaresConformalMaps::assign_pinned_uv()
{
	Up.resize(2 * pinned_len, 1);
...
void LeastSquaresConformalMaps::build_pinned_point()
{
...
int LeastSquaresConformalMaps::get_current_index(int refer_index)
{
...
Vector3d LeastSquaresConformalMaps::get_point(int refer_index)
{
	// because we switch two pinned points with last two points in V
	// but we haven't change its cooresponding facet(triangle)'s information(refered index inside it)
	// it is pinned point, its true information is at the last two points
...
void LeastSquaresConformalMaps::build_A_b()
{
...
void LeastSquaresConformalMaps::build_uv()
{
	// build uv from X
...
void LeastSquaresConformalMaps::lscg_method()
{
...
void LeastSquaresConformalMaps::sim_LDLT_method()
{
...
void LeastSquaresConformalMaps::sim_LDLT_improved_method()
{
...
void LeastSquaresConformalMaps::solve()
{
	std::cout << "-----------------------------------------------" << std::endl;
...
void LeastSquaresConformalMaps::init(Eigen::MatrixXd& v, Eigen::MatrixXi& f)
{
